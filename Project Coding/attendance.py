...
class Attendance:
    ...
    def __init__(self,root):
        ...
        attend_combo["values"]=("Status","Present","Absent","OnDuty")
        ...
        #create table 
        self.attendanceReport_left = ttk.Treeview(table_frame,column=("ID","Roll_No","Name","Department","Time","Date","Attend"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        table_frame.place(x=10,y=90,width=575,height=320)
    ...
    #Update button
        del_btn=Button(right_frame,command=self.update_data,text="Notification",width=12,font=("verdana",12,"bold"),fg="white",bg="navyblue")
    ...
    def update_data(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                Update=messagebox.askyesno("Notification","Do you want to Send Notification this Student Attendance!",parent=self.root)
                if Update > 0:
                    conn = mysql.connector.connect(user='root', password='',host='localhost',database='face_recognition',port=3306)
                    mycursor = conn.cursor()
                    mycursor.execute("update stdattendance set std_id=%s,std_roll_no=%s,std_name=%s,std_department=%s,std_time=%s,std_date=%s,std_attendance=%s where std_id=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_dep.get(),
                    ...
                    if not Update:
                        return
                messagebox.showinfo("Success","Successfully Sended!",parent=self.root)
                conn.commit()
                self.fetch_data()
                conn.close()
